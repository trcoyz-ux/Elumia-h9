...
#     "s3",
#     aws_access_key_id="YOUR_AWS_ACCESS_KEY_ID",  # Should be replaced with your access key
#     aws_secret_access_key="YOUR_AWS_SECRET_ACCESS_KEY",  # Should be replaced with your secret access key
...
def upload_medical_record():
    ...
    db.session.commit()
    return jsonify({"message": "Medical record uploaded successfully", "record_id": new_record.id}), 201

@medical_records_bp.route("/<int:record_id>", methods=["GET"])

@medical_records_bp.route("/all", methods=["GET"])
def get_all_records():
